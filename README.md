...
| `FREIZONE_PUSH_COALESCE_WINDOW` | `3s` | Collapses repeated push wakes for the same device into at most one per window. A wake carries no content and no reason, so several in a row say no more than one does — a lively group otherwise costs one push per message per member. The first wake to an idle device is never delayed; only follow-ups inside the window are merged, into a single wake at its end. Applies to both wake mechanisms. Set `0` to disable and go back to one push per message. Any [duration](https://pkg.go.dev/time#ParseDuration) up to `1m`. |
...
| `FREIZONE_REPORTS_ENABLED` | `true` | Whether members may report an account to you — see [docs/PROTOCOL.md §12](docs/PROTOCOL.md). Reports are **named**: you see who reported whom, so you can ask what happened. Nothing acts on a report by itself; you get a counter and the individual cases, and act with the block/delete powers you already have. Set to `false` to refuse them entirely, which clients discover via `GET /v1/server-status` and then don't offer the button. |
| `FREIZONE_REPORT_RETENTION_DAYS` | `90` | How long a report is kept, resolved or not. A counter that never falls becomes a permanent record for something that was never proven — and the profile-name evidence a report carries is deleted with it. Reports are also removed when either account is deleted. |
